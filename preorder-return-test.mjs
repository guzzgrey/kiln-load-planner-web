...
const result = await evaluate(`(() => {
  ...
  write(COMPLETED_KEY,[{id:'return-cycle-1',orderId:order.id,orderNumber:order.number,loadNumber:1,species:'Hemlock',quantities:{12:168,20:208},qualityLots:[{length:12,quantity:168,material:'Hemlock',quality:'unclassified'},{length:20,quantity:208,material:'Hemlock',quality:'unclassified'}],boards:376,bf:3088}]);
  ...
  const before={twelve:free(lot12),twenty:free(lot20)};
...
if (result.before.twelve!==0 || result.before.twenty!==144 || result.afterItem.twelve!==168 || result.afterItem.twenty!==144 || !result.afterItem.status.includes('168 boards at 12 ft returned') || result.afterStack.twelve!==168 || result.afterStack.twenty!==208 || !result.afterStack.status.includes('64 boards / 640.0 BF returned')) {
